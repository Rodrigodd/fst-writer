...
fn write_read_simple() {
    ...
    // held back by `SignalBuffer::request_flush`: the section has two time steps so far, one
    // short of what the reference acts on
    writer.flush().unwrap();
...
/// the reference, which drops a request below that outright rather than deferring it.
///
/// The section therefore stays open across the flush, and the time step at 30 joins it and reaches
/// the file — even though no value change follows it.
#[test]
